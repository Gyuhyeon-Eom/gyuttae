...
- 새 메시지·도움 요청·일정 등록/변경·할 일 수락/완료는 **앱 내부 알림함**에 저장합니다. 홈/대화는 약 7초 주기로 갱신합니다. 앱을 닫았을 때 휴대폰에 표시되는 OS 푸시는 아직 없습니다. 약속 전 알림함 예약은 아래 기능을 이용합니다.
...
- 대화 상단의 집 모양 **홈** 버튼으로 목록에 돌아갑니다. 네트워크가 끊겨도 홈 이동은 가능합니다.
- 실제 약속/할 일의 모아보기에서 **나에게 미리 알려주기**를 5분·30분·1시간·하루 전으로 설정합니다. 기본값은 꺼짐이고 본인 계정에만 적용됩니다.
- 서버가 1분 주기로 예약을 확인해 **앱 내부 알림함**에 저장합니다. 브라우저가 꺼져도 저장되지만 휴대폰 잠금 화면에 푸시되지는 않습니다.
- 일정 변경을 승인하면 새 시간에 맞춰 예약을 계산합니다. 완료·이미 지난 일정·권한이 사라진 방은 제외하며 같은 예약 시각에는 중복 전송하지 않습니다.
- 예약 시간이 이미 지나고 약속 전이라면 다음 확인 주기에 알림을 저장합니다. 분 단위 작업이므로 초 단위 정확성을 보장하지 않습니다.
- 검증: 예약 단위 테스트 4개, 기존 단위 검증 11개, 로컬 Worker의 실제 scheduled 핸들러 호출/중복 방지 통합 검증 1개 통과.

```sh
npm run test:reminders
npx wrangler dev --port 8917 --test-scheduled
# 다른 터미널에서
node --test tests/reminder-worker.test.mjs
```

예약 실행은 [Cloudflare Cron Triggers](https://developers.cloudflare.com/workers/configuration/cron-triggers/)를 사용합니다.

### 채팅 전송과 메시지 묶음

- 같은 방에서 같은 사람이 2분 이내에 보낸 일반 메시지는 이름과 아바타를 한 번만 표시합니다. 날짜 변경, AI 답변, 전송 실패가 사이에 있으면 새 묶음을 시작합니다.
- 글·사진은 전송 즉시 로컬 말풍선에 표시합니다. 전송 완료와 읽음은 서버 응답으로 구분합니다.
- 실패한 메시지는 해당 말풍선에서 수동 재전송합니다. 기존 요청 ID와 원문·사진을 유지해 서버의 중복 방지 처리를 사용합니다. AI 실패의 ‘다시 시도하기’와 메시지 전송 재시도는 별개입니다.
- 미전송 내용은 동일 탭의 sessionStorage에 보관합니다. 재접속 시 서버에 이미 저장됐는지 먼저 확인하고, 미확인 메시지는 자동으로 보내지 않습니다. 탭 종료나 브라우저 저장 공간 부족까지 보장하는 영구 보관함은 아닙니다.
- 회귀 검사: `npm run test:chat` (묶음 경계, 방 분리, 사진 보존, 요청 중복, 응답 유실, 독립된 실패 처리).

### 대화에서 약속 저장하기

- 메시지의 약속 후보에서 시각과 명시된 장소를 채웁니다. 상대 날짜(예: 토요일)는 임의로 확정하지 않으며, 날짜·시간은 함께 입력하거나 둘 다 미정으로 둡니다.
- 같은 원문 메시지에서 다시 저장하면 기존 항목을 반환합니다. 다른 가족의 동시 저장도 새 항목을 만들지 않습니다. 기존에 중복 저장된 데이터는 삭제하지 않습니다.
- 저장된 메시지에서 상세 화면을 바로 열 수 있습니다. 목록은 요약으로 표시하고, 상세에서 시간·장소·현재 버전 확인자와 변경 전후 내용을 보여줍니다. 알림과 변경 도구는 하단에 배치합니다.
- `node --test tests/collaboration.test.mjs tests/reminders.test.mjs`로 원문 중복 방지, 상세 접근 권한, 승인된 변경 및 버전별 확인 상태, 알림 동작을 검사합니다.
